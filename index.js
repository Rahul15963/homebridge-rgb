...
function powerOff(deviceId, topic) {
    client.publish(topic, 'R000G000B000')
}

function send(encodedData, topic){
 
    	client.publish(topic, 'e0');
		client.publish(topic, 'R' + FormatNumberLength(encodedData.red, 3) + 'G' + FormatNumberLength(encodedData.green, 3) + 'B' + FormatNumberLength(encodedData.blue, 3));
